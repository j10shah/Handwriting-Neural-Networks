# Name: Jay Shah
# Date: 03/31/2026
# Purpose: Task 4 - Train and evaluate the transformer-based model
...
def evaluate(model, loader): # Evaluate the model on the given data loader by calculating the average loss and accuracy, which allows for assessment of the model's performance on both training and testing datasets during the training process
    ...
    with torch.no_grad(): # Loop through the data in the loader, get the model's output for each batch, calculate the negative log likelihood loss, and count the number of correct predictions to compute overall performance metrics such as average loss and accuracy
        for data, target in loader: # Get the model's output for the input data, calculate the negative log likelihood loss, and determine how many predictions are correct to compute overall performance metrics
            output = model(data)
            total_loss += F.nll_loss(output, target, reduction='sum').item() # Calculate the total loss by summing the negative log likelihood loss for each batch, which will later be averaged over the entire dataset to get the average loss
    ...
    return avg_loss, accuracy # Return the average loss and accuracy for the given data loader, which can be used to track the model's performance over time during training and testing phases


def train_network(): # Train the transformer-based model on the MNIST dataset, evaluate its performance over multiple epochs, and generate training/testing loss and accuracy plots to analyze the model's learning progress and generalization capabilities
    batch_size = 64
    epochs = 5
    learning_rate = 0.01

    transform = transforms.Compose([transforms.ToTensor()]) # Define the transformation to convert images to tensors, which is necessary for inputting the data into the model during training and evaluation
    ...
    config = NetConfig()
    model = NetTransformer(config) # Create an instance of the transformer-based model using the defined configuration, which sets up the architecture and hyperparameters for the model that will be trained on the MNIST dataset
    ...
    for epoch in range(epochs): # Train the model for a specified number of epochs, evaluating its performance on both training and testing datasets at the end of each epoch to track the learning progress and generalization capabilities of the transformer-based model on the MNIST dataset
        model.train()
        ...
        train_loss, train_acc = evaluate(model, train_loader) # Evaluate the model on the training dataset at the end of each epoch to compute the average loss and accuracy, which provides insights into how well the model is fitting the training data and helps identify any issues such as overfitting or underfitting
...
def plot_metrics(train_steps, train_losses, test_steps, test_losses, # Generate and save plots for training/testing loss and accuracy over time to visualize the model's performance during training, which allows for analysis of the learning progress and generalization capabilities of the transformer-based model on the MNIST dataset
                 train_accs, test_accs):
